...
def get_transactions_graph(address):
    ...
    transactions_url = make_api_url("account",
                                    "txlist", 
                                    address, 
                                    startblock=0, 
                                    endblock=99999999, 
    ...
    internal_tx_url = make_api_url("account",
                                    "txlistinternal",
                                    address,
                                    startblock=0,
                                    endblock=99999999,
...
def get_normal_transactions(address, index):
...
def get_internal_transactions(address, index, startblock):
    print('Making API calls')
    # offset = get_offset(page)
    ...
    columns = ['to', 'from', 'value', 'gas', 'time', 'nonce']
    df = pd.DataFrame(lst, columns=columns)
    df.to_feather('dai_internal' + str(index) + '.feather')
    print(f'Download finished. Last transaction time: {time} and last block number: {block_number}')
    return block_number